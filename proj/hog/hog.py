...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while scores[0] < goal and scores[1] < goal:
        num_rolls = strategies[player](scores[player], scores[other(player)])
        scores[player] += take_turn(num_rolls, scores[other(player)], dice)
...
def announce_highest(who, previous_high=0, previous_score=0):
    ...
    def say(score0, score1):
        ...
        gain = scores[who] - previous_score
        current_high = previous_high

        if gain > previous_high:
            print(f"{gain} point{'s' if gain > 1 else ''}! That's the biggest gain yet for Player {who}")
            current_high = gain
        
        return announce_highest(who, current_high, scores[who])
    return say
    # END PROBLEM 7
